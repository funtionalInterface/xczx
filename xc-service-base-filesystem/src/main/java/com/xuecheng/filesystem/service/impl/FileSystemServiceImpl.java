...
public class FileSystemServiceImpl implements FileSystemService {
    ...
    private String fdfsUpload(MultipartFile multipartFile) {
        ...
        try {
            ...
            // 上传文件
            return storageClient1.upload_file1(fileBytes, extStr, null);
        } catch (Exception e) {
            e.printStackTrace();
            ExceptionCast.cast(FileSystemCode.FS_UPLOADFILE_SERVERFAIL);
        }
    ...
    public ResponseResult deleteFile(String picPath) {
        if (StringUtils.isBlank(picPath) || "undefined".equals(picPath)) ExceptionCast.cast(CommonCode.INVALID_PARAM);
        try {
            initFdfsConfig();
            TrackerClient trackerClient = new TrackerClient();
            TrackerServer trackerServer = trackerClient.getConnection();
            //得到 Storage 连接信息
            StorageServer storeStorage;
            storeStorage = trackerClient.getStoreStorage(trackerServer);
            //获取 Storage 客户端
            StorageClient1 storageClient1 = new StorageClient1(trackerServer, storeStorage);
            int i = storageClient1.delete_file1(picPath);
            if (i != 22) {
                Long aLong = fileSystemRepository.deleteByFileId(picPath);
                if (aLong > 0) return new ResponseResult(CommonCode.SUCCESS);
            }
        } catch (Exception e) {
            e.printStackTrace();
